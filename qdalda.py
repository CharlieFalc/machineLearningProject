...
class LDA(QDA):
    ...
    def _finishTrain(self):
        # self.sigmaMean = np.sum(np.array(self.sigma) * np.array(self.prior)[:,np.newaxis,np.newaxis], axis=0)
        sigmaSum = self.sigma[0] * self.prior[0]
        for i in range(1,len(self.sigma)):
            sigmaSum += self.sigma[i] * self.prior[i]
        self.sigmaMean = sigmaSum
